        ...
        (let* ((cur-dir (file-name-directory load-file-name))
               ...
               (apply 'start-process "pymacs" buffer
                      ...
                      (append
                       ...
                       (mapcar* 'expand-file-name pymacs-load-path (let (cur-dirs)
                                                                       (dotimes (i (length pymacs-load-path))
                                                                         (message "%d" i)
                                                                         (setq cur-dirs (cons cur-dir cur-dirs)))
                                                                       cur-dirs))))))
          (message "load-file-name: %s" (file-name-directory load-file-name))
          (message "cur-dir: %s" cur-dir)
          (message "%s" (mapcar* 'expand-file-name pymacs-load-path (let (cur-dirs)
                                                                       (dotimes (i (length pymacs-load-path))
                                                                         (message "%d" i)
                                                                         (setq cur-dirs (cons cur-dir cur-dirs)))
                                                                       cur-dirs)))
